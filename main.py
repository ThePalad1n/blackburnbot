...
@bot.command(name='rerarity', help='List of quotes and how rare')
async def rerarity(ctx):
    if ctx.message.content.startswith('re rerarity'):
      await ctx.message.channel.send("Total: 91\n Common: 29\n Uncommon: 20\n Rare: 15\n Epic: 11\n Legendary: 8\n God: 3\n Hidden: ???\n")
...
@bot.command(name='cjtrarity', help='List of quotes and how rare')
async def cjtrarity(ctx):
    if ctx.message.content.startswith('re cjtrarity'):
      await ctx.message.channel.send("Total:30 \n Common: 8\n Uncommon: 6\n Rare: 6\n Epic: 5\n Legendary: 5\n God: 2\n Hidden: ???\n")

@bot.command(name='cjt', help='The quotes youve always wanted')
async def cjt(ctx):
    if ctx.message.content.startswith('re cjt'):

      dict = {"Whats goin on guys, CJT here." : 5, "Kobra Kai" : 4, "Cum" : 1, "Cum Burger" : 2, "Columbus Kai" : 4, "Burger\n Bus\n" : 6, "*COUGH" : 2, "*WHEEZE" : 3, "Krazy Kai" : 1, "Kris Kringle" : 4, "OH\n My Christmas" : 6, "Antidrunk\n Active\n YEEEOWAH" : 3,"King Kai" : 1, "Cheeseburger Eddie" : 4, "Cheeseburger Walrus" : 3, "Lemme suc ur dic" : 3, "Let me see it" : 1, "Imma twist ur dick" : 1, "YEEEEEEOOOWAH" : 6, "The Biiiiiig\n Bus" : 5, "Wanna makeout?":3, "Omg\n Im gonna see a woman\n One I can penetrate" : 5, "*hand on gents" : 2, "Billy." : 2, "Wheres blobo": 7, "I\n Love Cats" : 1, "Kringle Kris" : 5, "Let me fuck you" : 1, "11/10" : 2, "Im 13/10\n and im never 13 out of 10" : 3, "Kaioken" : 4, "value" : 5, "Mmm Hmmm" : 3, "im gonna cuck ur soc" : 3, "big soc" : 2, "big suc" : 1, "meat soc" : 4}		
    ...
    if (y == 7):
        await ctx.message.channel.send(x + " (Secret Rare)\n")
        myid = '<@189552961389592578>'
        await ctx.message.channel.send(myid)
    elif(y == 1):
        await ctx.message.channel.send(x + " (Common)")
    elif(y == 2): 
        await ctx.message.channel.send(x + " (Uncommon)")
...
@bot.command(name='cjthere', help='Word Masher')
async def cjthere(ctx):
    if ctx.message.content.startswith('re cjthere'):

      xxx = ["Cobra", "Cum", "Cheeseburger", "King", "Eldridge", "Burger"]
      xx = random.choice(xxx)
      yyy = ["Kai", "Eddie", "Walrus", "Suc", "Bus"]
      yy = random.choice(yyy)

      await ctx.message.channel.send(xx + " " + yy)
...
@bot.command(name='shutup', help='bot on bot action')
async def shutup(ctx):
    if ctx.message.content.startswith('re shutup'):
        await ctx.message.channel.send(";compile\n\'\'\'c\n #include <stdio.h>\n int main() {\n printf(\'Aye yo shut up!\');\n }\n\'\'\')")
